...
export class HappyRedPacket extends ethereum.SmartContract {
  ...
  check_availability(id: BigInt): HappyRedPacket__check_availabilityResult {
    let result = super.call(
      "check_availability",
      "check_availability(uint32):(address,uint256,uint256,uint256,bool,uint256)",
  ...
  claim(id: BigInt, proof: Array<Bytes>): BigInt {
    let result = super.call("claim", "claim(uint32,bytes32[]):(uint256)", [
      ethereum.Value.fromUnsignedBigInt(id),
  ...
  try_claim(id: BigInt, proof: Array<Bytes>): ethereum.CallResult<BigInt> {
    let result = super.tryCall("claim", "claim(uint32,bytes32[]):(uint256)", [
      ethereum.Value.fromUnsignedBigInt(id),
      ethereum.Value.fromFixedBytesArray(proof)
    ]);
    if (result.reverted) {
  ...
  nonce(): BigInt {
    let result = super.call("nonce", "nonce():(uint32)", []);

    return result[0].toBigInt();
  }

  try_nonce(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("nonce", "nonce():(uint32)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
