...
contract HappyRedPacket is Initializable {
    ...
    event RefundSuccess(bytes32 id, address token_address, uint remaining_balance, bytes32 lock);
    ...
    function _claim(bytes32 _id, bytes32[] memory proof) internal returns (uint claimed) {
        bytes32 id = _id;
        RedPacket storage rp = redpacket_by_id[id];
        Packed memory packed = rp.packed;
        // Unsuccessful
